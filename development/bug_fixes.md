...
### BUG-005: 成本树入库主键冲突 ✅
**原因**：`cost_items` 表使用 `idx` (0-indexed) 作为主键，但每次保存成本树时都从 0 开始计数，导致多个会话或不同视角的树节点 ID 重复
**修复**：
- `backend/app/services/cost_variance_service.py`: 使用 `md5(session_id + view + item_id + idx)` 生成唯一的数字 ID 作为主键
**教训**：在设计数据表时，如果数据具有层级或多维度特性，简单的自增索引往往不足以作为全局唯一标识
...
### UX-001: 气泡图长尾数据展示与象限区分问题 ✅
**现象**：APV 数据跨度极大（长尾分布），使用默认线性轴导致大部分气泡挤在左下角；且 0 点坐标轴无法体现业务上的"高/低"概念
**修复**：
- 引入**动态阈值**（默认中位数），替代固定的 0 点分割
- 实现**交互式四象限着色**，直观区分 Core/Potential/Stable/Ignore
- 动态计算坐标轴 min/max，避免大量留白
**教训**：对于业务分析图表，"可解释性"（如象限含义）比纯粹的"数据准确性"（如绝对坐标）更重要
